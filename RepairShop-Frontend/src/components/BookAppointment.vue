      ...
      <div class="inputWidth">
          ...
          <b-form-group label="Select a service" class="mt-4">
            ...
            >
              {{
                s.name +
                  ", for " +
                  displayDuration(s.duration) +
                  " ($" +
                  s.price +
                  ")"
              }}
            </b-form-radio>
            <p v-show="services.length === 0">
        ...
        <div v-if="formSection == 2">
          <b-form-group label="Select a date and time" class="mt-4">
            ...
            >
              {{
                displayDateTime(t.startDateTime) +
                  " to " +
                  displayDateTime(t.endDateTime)
              }}
            </b-form-radio>
            <p v-show="availableTimes.length === 0">
              There are currently no time slots available.
            </p>
          </b-form-group>

          <p class="mt-3">
            <b>Selected start time:</b>
            {{ displayDateTime(start.startDateTime) }}
          </p>

          <b-form-group
            label="Or enter a future date to see possible schedules for that week:"
          ...
          >
            ...
            <b-form-input
              ...
              class="ml-3"
            ></b-form-input>
            <b-button variant="secondary" class="ml-3" @click="updateTargetDate"
              >Go</b-button
            >
          </b-form-group>
        ...
        <div v-if="formSection == 3">
          ...
          <b-button variant="secondary" class="mt-3 mr-3" @click="toPart2"
            >Back</b-button
          >
          <b-button variant="primary" class="mt-3" @click="book"
            >Book now</b-button
          >
        </div>
...
export default {
  ...
  methods: {
    ...
    getPossibleAppointments() {
      axios
        .get(BACKEND + POSSIBLE_APPOINTMENTS_ENDPOINT, {
          params: {
    ...
    updateTargetDate() {
      ...
      if (this.targetDate.length != 10 || newTarget == "Invalid Date")
        this.error = "Please select a valid date";
      else if (newTarget < new Date())
        this.error = "Please enter a future date";
      else {
        this.error = "";
        this.getPossibleAppointments();
      }
    ...
    book() {
      axios
        .post(
          BACKEND + CREATE_APPOINTMENT_ENDPOINT,
          {
